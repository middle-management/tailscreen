...
# new XCTest suites that exercise the share pipeline beyond what unit tests
# cover — synthetic frames over real tsnet, real capture-helper subprocess,
# picker-helper subprocess smoke.
#
# The synthetic-frames test is CI-eligible; the capture-helper and picker
# tests self-skip on GITHUB_ACTIONS / CI runners. Set
# TAILSCREEN_ALLOW_CAPTURE_TEST=1 to force the capture-helper test locally
# even if those env vars are set.
