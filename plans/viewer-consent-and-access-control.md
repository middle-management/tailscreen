...
- **"Deny & Block"/"Always Allow" stick when StableNodeID is unresolved.** The intent is queued (the shared `ViewerRosterDecision.PendingIntents`, keyed by the row's `ip:port` id) and persisted the instant a roster snapshot carries the resolved StableNodeID (`AppState.noteRoster`, fed BOTH rosters together and coalesced to the end of the main-actor turn so an Accept's two callbacks land before the queue is pruned). Deny-while-unresolved keeps the peer parked (denied access) so the server keeps resolving; the split-button menu items are no longer disabled pre-resolution.
