...
**`audio.summary`** mirrors it one release later, from the pure `VoiceStats.audioSummaryFields` (`AudioSummaryTests`), on mac `VoiceChannel` and portable `VoiceDownlink`. Carries what was playing (`voice_streams` per-window, not sticky; `system_audio_in`/`system_audio_out`, `mic_on`, `jitter_target`, `burst_depth`, `output_device`) because zero counters mean nothing without knowing what should have produced sound; separates system-audio clipping from voice clipping; **omits** (never zeros) `overruns`/`underruns` on a host with no playback queue — a false zero reads as "nothing dropped" instead of "nobody counted". Row recorded for every window audio is running, whether or not a counter moved — but a window with no audio activity at all records nothing (the lifecycle events already say audio should be running). Sharer's row also carries `audio_packets_in`/`audio_rejected_in` (inbound viewer audio) since every other field describes outbound-only traffic. **System audio is two fields, not one.** `system_audio_in` is inbound PT 99, which a sharer never receives — the first cut had only that, under the name `system_audio`, so the machine that turned system audio ON reported `false` for every row while the viewer opposite reported `true`: one pair, two contradictory answers. `system_audio_out` is pushed by the host (system audio leaves through the capture helper, so nothing on the voice path can see it), alongside `output_device`. **Read `burst_depth` against `jitter_target`**: far apart is the shape that overruns and underruns at the same time, and is what smoothed jitter cannot show — an rc.16 pair had `jitter_target` pinned at 3 with jitter a calm 32–39 ms while the receiver dropped 9 % of the frames it had been handed and starved 327 times. `burst_depth` is the reading the target is now sized from (`VoiceReceiveDecisions.PlayoutBacklog`).
