...
Those commits are the former `Patches/*.patch` series, converted one-to-one (subjects match the old filenames; the sections below still refer to them by their historical patch numbers). They add things like a `Foundation` import, glue imports for C-bridge types, `send`/`receive` on connections, public `logout`, listener poll-timeout handling, the `tsnet ListenPacket` / `PacketListener` Swift wrapper used by the UDP video path, a short-write-safe `OutgoingConnection.send` loop (023 — replaced the app-side reflection hack that reached the private fd), Linux portability gates (022 — Combine→AsyncStream fallback, Glibc syscall shim, `FoundationNetworking` imports, SOCKS-free direct-loopback LocalAPI), the **Windows bridge seam** (024 — see below), the **Windows Go-runtime start** (026 — see below), and the `tailscale.com` → v1.102.3 bump.
