...
- **Spawn.** The parent `Process()`-execs the same binary with `--capture-helper`. Stdin and stdout are pipes; stderr streams through to the parent's terminal. Quality knobs (fps cap, codec preference, bandwidth ceiling — see `QualitySettings`) travel as spawn-time env vars (`TAILSCREEN_FPS_CAP` / `TAILSCREEN_CODEC_PREF` / `TAILSCREEN_MAX_BITRATE`, same pattern as `TAILSCREEN_FORCE_H264`, which still wins over the codec preference); the child env is always seeded from the parent's before overlaying (`Process.environment` replaces, it doesn't merge).
