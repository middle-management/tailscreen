...
- `make test-protocol` — builds + smoke-tests the portable TailscreenKit package with no Apple frameworks. Also runs on Linux, and is how you reproduce a `linux-protocol` CI failure locally. It builds **two** Go archives first: `libtailscale.a` (the package's `TailscreenSharerTests` bundle links `TailscreenSharer` → `TailscaleKit`, so the archive is a link-time input even though no test calls tsnet) and `libtailscreen.a` (the public Go SDK as a c-archive, linked by `TailscreenDifferentialTests` — the suite that drives the shipping Swift pipeline and the Go SDK with identical seeded input and asserts identical output at every step; the `CTailscreen` systemLibrary resolves it via `sdk/go/libtailscreen.pc`, which is why the Makefile's `PKG_CONFIG_PATH` now also carries `sdk/go`).
