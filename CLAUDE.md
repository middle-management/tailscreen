...
- **Wire.** Framed binary — `[type:1][len:4 BE][payload:N]`. Message types: encoded access unit (AVCC), parameter sets (H.264 SPS/PPS or HEVC VPS/SPS/PPS), preview thumbnail, **heartbeat**, log line, fatal, user-stopped; control commands request-keyframe, set-bitrate, **set-frame-interval** (`0x04`, the fps-ladder lever — reconfigures the SCStream's `minimumFrameInterval` live in the helper, never the main process), content-filter, shutdown. The heartbeat is a ~1 Hz liveness ping emitted off *any* delivered SCStream sample (including the `.idle` frames a static screen still produces, which carry no pixel buffer and so produce no AUs) — a content-independent proof the capture pipeline is alive. `HelperFrameWriter` is lock-serialized because it's now written from three threads: the encoder output thread (AUs/params), the MainActor (previews), and the SCStream delegate queue (heartbeats).
