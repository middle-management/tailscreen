...
│   ├── PortalCaptureKit/       # C shim over xdg-desktop-portal ScreenCast
│   │                           #   (libdbus) + PipeWire — the WAYLAND-capable
│   │                           #   Linux capture path, and the only one that
│   │                           #   can share a single window or app. NOT yet
│   │                           #   wired into Apps/linux and no frame has been
│   │                           #   captured through it: its CI leg
│   │                           #   (linux-portal) is a compile/link/D-Bus-
│   │                           #   protocol gate and can never be a capture
│   │                           #   gate, because the portal is defined by a
│   │                           #   consent dialog a person clicks. Hands back
│   │                           #   BGRA and converts nothing — the portable
│   │                           #   BGRAToI420 owns colour, same split as
│   │                           #   WGCCaptureKit → TailscreenSharerWGC. Read
│   │                           #   its README's "What is proven" table before
│   │                           #   trusting any of it
...
│   ├── WASAPIKit/              # C++ shim over WASAPI shared-mode rendering AND
│   │   │                       #   capture — the WINDOWS viewer's audio-playback
│   │   │                       #   backend (what ALSAKit is on Linux) plus its
│   │   │                       #   microphone (`WASAPI.Recorder`, mono at the
│   │   │                       #   device's own rate — the channel mix is here,
│   │   │                       #   the RATE conversion is MonoPCMConverter's).
│   │   │                       #   Nothing to install (WASAPI ships with
│   │   │                       #   Windows). The `linux-wasapi` CI leg covers
│   │   │                       #   only the pure arithmetic — there is no WASAPI
│   │   │                       #   to gate on off Windows; see its README
...
│   │                           #   its backends render checked menu rows). Its
│   │                           #   AnnotationToolbar + RemoteControlBar are why
│   │                           #   the Windows viewer's drawing/control layer
│   │                           #   was mostly wiring rather than new UI.
