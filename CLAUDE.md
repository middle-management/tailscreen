...
Test-only seams added for the above: `TailscaleScreenShareClient.onDecodedFrameForTesting`, `.sendPLIForTesting()`, `.extractParameterSets` (internal, not private); `TailscaleScreenShareServer.onPLIRecordedForTesting`, `.injectSyntheticParameters`, `.broadcastForTesting`, `.nextAdaptiveBitrate`, `.rewriteRTPHeader` (internal, not private), plus the pure static decision funcs (`audioRelayDecision`, `staleAddrs`, `appendingPLI`, `helperLooksHung`, `classifyHelperExit`, `slidingWindowCrashCount`, and `VoiceChannel`'s `decoderGateAction` / `gapAction` / `jitterBufferTarget` / `shouldLogClamp` / `clampToUnitRange`); `VoiceChannel` also exposes DEBUG-only `decoderFailuresForTesting` / `injectDecoderFailureForTesting`. Shared bring-up helpers live in `TailscreenE2EHelpers.swift` (`encodeSyntheticAUs`, multi-dir `makeStateDirs`).
