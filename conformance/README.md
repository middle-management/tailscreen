...
cannot express a clock-driven interleaving: `make test-differential`
(`Packages/TailscreenDifferential`) links the Go SDK's c-archive
(`make libtailscreen`) and drives both implementations with identical
seeded, clock-injected input, asserting identical output at every step.
