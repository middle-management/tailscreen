...
The same release carries the **Linux** build (x86_64 + arm64 AppImages, plus an arm64 tarball) and the **Windows** build (x64 + arm64 zip, and an MSIX with its signing cert for a trust-once install) — see the [Install docs](https://tailscreen.dev/install/) for those.
