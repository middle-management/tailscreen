...
public enum CaptureHelperWire {
    ...
    public enum OutType: UInt8, CaseIterable {
    ...
    public enum InType: UInt8, CaseIterable {
...
public final class HelperFrameWriter: @unchecked Sendable {
    ...
    public func writeParameterSets(codec: UInt8, width: Int, height: Int, paramSets: [Data]) {
        var payload = Data()
    ...
    public func writeFirstFrame() { write(type: .firstFrame, payload: Data()) }

    public func writePreviewJPEG(_ jpeg: Data) { write(type: .previewJPEG, payload: jpeg) }

    public func writeUserStopped() { write(type: .userStopped, payload: Data()) }

    public func writeLog(_ line: String) {
        guard let data = line.data(using: .utf8) else { return }
        write(type: .logLine, payload: data)
    }

    public func writeFatal(_ message: String) {
...
/// asynchronously.
public struct HelperFrameReader {
    public init(handle: FileHandle) { self.handle = handle }
...
public final class HelperControlWriter {
    private let handle: FileHandle
    public init(handle: FileHandle) { self.handle = handle }

    public func sendKeyframeRequest() { write(type: .requestKeyframe, payload: Data()) }

    public func sendBitrate(_ bitrate: Int) {
        var payload = Data()
        payload.appendBE(UInt32(max(0, bitrate)))
        write(type: .setBitrate, payload: payload)
    }

    public func sendContentFilter(_ data: Data) {
        write(type: .contentFilter, payload: data)
    }

    /// Toggle system-audio emission in the helper; see `InType.setAudioEnabled`.
    public func sendAudioEnabled(_ on: Bool) {
        write(type: .setAudioEnabled, payload: Data([on ? 1 : 0]))
    }

    public func sendFrameInterval(_ fps: Int) {
        var payload = Data()
        payload.appendBE(UInt32(max(1, fps)))
        write(type: .setFrameInterval, payload: payload)
    }

    public func sendShutdown() { write(type: .shutdown, payload: Data()) }
