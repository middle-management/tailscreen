...
public struct Annotation: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: UUID, tool: AnnotationTool, points: [CGPoint], color: RGBA, width: Double) {
        self.id = id
        self.tool = tool
        self.points = points
        self.color = color
    ...
    public static let defaultColor = RGBA(r: 1.0, g: 0.1, b: 0.15, a: 1.0)
    public static let defaultWidth: Double = 3.0
}
...
extension Annotation.RGBA {
    ...
    public static let palette: [Annotation.RGBA] = [
...
public enum AnnotationOp: Codable, Sendable, Equatable {
    case add(Annotation)
    case undo(UUID)
    case clearAll
