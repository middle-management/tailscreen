...
///
/// **Not** `TailscreenViewerGtk.AnnotationStore`, the GTK viewer's DRAWING
/// store (live stroke tracking, tool mode, this participant's colour) — an
/// earlier version shared the same name and broke that viewer's build via
/// import shadowing.
///
/// macOS's `AnnotationCanvasModel` does this *and* owns local drawing; a
/// sharer that only displays what viewers send needs none of that, so this
/// is just the shared half: apply an op, drop what aged out, hand back what
/// to draw.
///
/// Value type with an injected clock — the caller owns the thread and tests
/// must not sleep.
public struct ReceivedAnnotations: Sendable {
    /// How long a `.click` marker stays up. Matches the macOS overlay's
    /// animation so it vanishes at the same moment on both machines.
    public static let clickLifetimeNs: UInt64 = 800_000_000

    /// Whether a tool's strokes disappear on their own, and after how long.
    /// Only `.click` is ephemeral today. Mirrors
    /// `AnnotationCanvasModel.ephemeralLifetime(for:)` on macOS.
    ...
    /// Apply one op. Returns whether anything changed, so a caller can skip
    /// a redraw (e.g. `undo` of an unknown id).
    @discardableResult
